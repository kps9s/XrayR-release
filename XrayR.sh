...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/kps9s/XrayR-release/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/kps9s/XrayR-release/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_menu() {
    echo -e "
  ${green}XrayR 后端管理脚本，${plain}${red} 修改版${plain}
